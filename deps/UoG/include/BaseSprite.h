...
namespace UG
{
	...
	public:
		...
		BaseSprite(const char* a_pTexture, float a_fWidth, float a_fHeight, const UG::Vertex* a_vertexData, glm::vec4 a_v4colour = glm::vec4(1.f, 1.f, 1.f, 1.f));
		...
		void	SetVertexData(UG::Vertex* a_vertexData);
		const	UG::Vertex* GetVertexData() const;
	...
	inline void	BaseSprite::SetVertexData(UG::Vertex* a_vertexData)
	{
		memcpy(&m_aoVertices, a_vertexData, sizeof(UG::Vertex) * 4);
	}

	inline const UG::Vertex* BaseSprite::GetVertexData() const
	{
		return static_cast<const UG::Vertex*>(m_aoVertices);
	}
